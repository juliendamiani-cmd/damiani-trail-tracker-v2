...
function changeRace(){active=Number($("raceSelect").value);times=JSON.parse(localStorage.getItem("trail_times_"+active)||"[]");if(!times[0])times[0]=race().start;loadRaceFields();saveAll();render()}function loadRaceFields(){$("startTime").value=race().start;$("goalTime").value=race().goal}function saveSettings(){race().start=$("startTime").value;race().goal=$("goalTime").value;if(!times[0])times[0]=race().start;saveAll();render()}function pointNext(){let i=Math.min(last()+1,points().length-1);times[i]=now();localStorage.removeItem(manualPositionKey());saveAll();render()}function resetTimes(){if(confirm("Réinitialiser la course ? Tous les pointages et toutes les informations d’état partagées avec les proches seront supprimés.")){times=[];times[0]=race().start;localStorage.removeItem(CLOSE_HISTORY_KEY);localStorage.removeItem("state_notes");localStorage.removeItem(manualPositionKey());if($("closeMessage"))$("closeMessage").value="";saveAll();render();renderFamilyDashboard();alert("La course a été réinitialisée. Les pointages et les informations d’état destinées aux proches ont été supprimés.")}}function toggleRaceMode(){$("raceMode").classList.toggle("hidden");scrollTo({top:0,behavior:"smooth"});render()}function toggleHard(){hard=!hard;render()}function toggleCloseMode(){$("closeMode").classList.toggle("hidden");scrollTo({top:$("closeMode").offsetTop-70,behavior:"smooth"});render()}function toggleEditor(){$("editor").classList.toggle("hidden");loadEditor()}
...
async function shareApp(){let data={title:"Suivre Julien — Damiani Trail Tracker",text:familySummary(),url:location.href};try{if(navigator.share)await navigator.share(data);else{await navigator.clipboard.writeText(`${data.text}\n${data.url}`);alert("Lien et résumé copiés.")}}catch(e){}}
...
async function pushSync(silent=false){
  ...
  try{syncBusy=true;if(!silent)setSyncUi("pending","Envoi…");const r=await fetch(ep,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(currentSyncPayload())});if(!r.ok)throw new Error(`HTTP ${r.status}`);setSyncUi("online","Synchronisé à "+now());return true}catch(e){setSyncUi("error","Erreur de synchronisation");if(!silent)alert("Impossible d’envoyer les données. Vérifie l’adresse Firebase et les règles de la base.");return false}finally{syncBusy=false}
}
...
function familyShareUrl(){const s=syncSettings();if(!s.url||!s.key)return"";const u=new URL(location.href);u.searchParams.set("mode","family");u.searchParams.set("firebase",normalizeFirebaseUrl(s.url));u.searchParams.set("sync",safeSyncKey(s.key));return u.toString()}
async function shareSyncLink(){const url=familyShareUrl();if(!url){alert("Active d’abord la synchronisation.");return}const data={title:"Suivre Julien en direct",text:"Suivi en direct de Julien sur l’UT4M 50 Belledonne",url};try{if(navigator.share)await navigator.share(data);else{await navigator.clipboard.writeText(url);alert("Lien proches copié.")}}catch(e){}}
...
function loadSyncFromUrl(){const q=new URLSearchParams(location.search),url=q.get("firebase"),key=q.get("sync");if(url&&key){localStorage.setItem(SYNC_SETTINGS_KEY,JSON.stringify({url:normalizeFirebaseUrl(url),key:safeSyncKey(key)}));localStorage.setItem(ROLE_KEY,"family");history.replaceState({},"",location.pathname);return true}return false}
